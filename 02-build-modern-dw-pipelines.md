...
2. In the Develop menu, expand **Notebooks** and select the notebook named `Exercise 2 - Ingest Sales Data`.
   ...
   ![The Exercise 2 - Ingest Sales Data notebook is selected and highlighted in the list of notebooks.](media/ex02-notebooks-ingest.png "Select the exercise 2 notebook")

3. If not already attached, attach your Spark Compute by selecting it from the **Attach to** drop-down list.

   ![The Spark pool is selected in the Attach to drop-down.](media/ex02-notebooks-toolbar-attach-to.png "Attach the Spark compute")

4. Ensure **Spark (Scala)** is selected in the Language drop-down list.

   ![Spark (Scala) is selected in the Language drop-down.](media/ex02-notebooks-toolbar-language.png "Select the Spark Scala language")

5. At the bottom left corner of the open notebook, select **Configure session**.

   ![The Configure session link is highlighted.](media/ex02-notebooks-configure-session-link.png "Select configure session")

6. In the Configure session dialog, ensure the following is configured:
   ...
   ![In the Configure session dialog, Step 1 indicated to set Executors 3, Step 2 indicates to set Executor size to Medium, and Step 3 says to select the Apply button.](media/ex02-notebooks-configure-session.png "Set the configuration values")
...
8. Next, in the first cell of the notebook, **Cell 1**, locate the variable named `uniqueId` on line 8.
   ...
   ![The uniqueId variable on line 8 of Cell 1 is highlighted.](media/ex02-notebooks-configure-unique-id.png "Update the uniqueId variable")
...
9. Update the value of the `uniqueId` variable, setting it to the unique identifier you were assigned for this workshop. This will be the _UNIQUEID_ component of your username, as mentioned above. For example:
   ...
   ![The uniqueId variable is highlighted and its value is set to an example value of "23456".](media/ex02-notebooks-configure-set-unique-id.png "Again, be sure to set the uniqueId variable")
...
10. Select **Run all** from the notebook toolbar to execute each cell within the notebook. It can take a minute or two for the Spark session to start, continue on to review the workbook logic.
...
11. While the notebook is running, let's take a look at what each cell is doing.
    ...
    ![Cell 1 of the notebook is displayed.](media/ex02-notebooks-ingest-cell-1.png "Review Cell 1")
    ...
    ![Cell 2 of the notebook is displayed.](media/ex02-notebooks-ingest-cell-2.png "Review Cell 2")
    ...
    ![Cell 3 of the notebook is displayed.](media/ex02-notebooks-ingest-cell-3.png "Review Cell 3")
...
12. When Cell 2 finishes running, take a moment to review the associated output.
    ...
    ![The output from the execution of Cell 2 is displayed, with the result of the show(5) command shown first, followed by the output from the printSchema() command.](media/ex02-notebooks-ingest-cell-2-output.png "Review the Cell 2 output")
...
20. Close the `Exercise 1 - Read with SQL on Demand` tab, do not save the file

21. Close the `SQL Script 1` tab, do not save the file
...
5. Close the `Exercise 2 - Bonus Notebook with CSharp` tab, select **Discard changes**.
...
18. Let's now take a moment to review the pipeline.
    ...
    - Selecting a pipeline opens the pipeline canvas, where you can review and edit the pipeline using a code-free, graphical interface. This view shows the various activities within the pipeline and the links and relationships between those activities.

    - Your cloned `Exercise 2 - Enrich Data` pipeline contains two activities
...
19. Let's now take a closer look at each of the activities within the pipeline. On the canvas graph, select the **Copy data** activity named `Import Customer dimension`. 

    - Below the graph is a series of tabs, each of which provides additional details about selected the activity. 
    - The **General** tab displays the name and description assigned to the activity, along with a few other properties.
...
44. Before running the pipeline there is one more change we need to make. As mentioned above, the data flow depends on the data written by the copy activity, so you are going to add a dependency between the two activities.

45. In the data flow canvas graph, select the green box on the right-hand side of the **Copy data** activity and drag the resulting arrow up onto the **Mapping Data Flow** activity.
...
46. This creates a requirement that the **Copy data** activity completes successfully before the **Mapping Data Flow** can execute, and enforces our requirement of the Synapse Analytics table being populated before running the data flow.
...
47. The last step before running the pipeline is to publish the changes you have made. Select **Publish all** on the toolbar.
...
48. On the **Publish all** dialog, select **Publish**.  Be sure that you are only publishing your two specific identifier Pipeline and Data flow changes. Close any other windows and select **Discard changes** before publishing.
...
49. Within a few seconds, you will receive a notification that the publish completed. Select **Dismiss** in the notification.

    ![The publishing completed notification is displayed.](media/ex02-publishing-completed.png "Publishing completed")

50. Your pipeline is now ready to run. Select **Add trigger** then **Trigger now** on the toolbar for the pipeline.
...
51. Select **OK** on the Pipeline run dialog to start the pipeline run.
...
52. To monitor the pipeline run, move on to the next task.
...
11. When the pipeline execution completes, all activities runs will reflect a status of Succeeded. It can take up to 10 minutes for the activities to execute.
...
12. Hover your mouse cursor over the **Enrich Customer Data** activity and select the **Details** icon that appears.

    > **Note** The activity must be in a **Succeeded** state for the details icon to display. It can take up to 10 minutes for the activity to execute.
...
15. Close the data flow activity Details dialog by selecting the **X** on the right-hand side of the toolbar.
...
16. Back on the `Exercise 2 - Enrich Data_{ID}` pipeline run screen, switch to the `Gantt` view. This view provides a graphical representation of the run times of the various activities within the pipeline.
...
1. In the navigation, under **Activities**, select **Apache Spark applications**
...
2. On the Apache Spark applications page, select the **Submit time** value and observe the available options for limiting the time range for Spark applications that are displayed in the list. In this case, you are looking at the current run, so ensure **Last 24 hours** is selected and then select **OK**.
...
3. From the list of Spark applications, select the first job, which should have a status of `In progress`.
...
4. On the **Log query** screen, you will see a detailed view of the job, broken into three different sections.
...
5. In the graph section, the initial graph displayed is that of the overall job progress. Here, you can see the progress of each stage.
...
6. Selecting any individual stage from the graph opens a new browser window showing the selected stage in the Spark UI, where you can dive even deeper into the tasks that make up each stage. Select **Stage 5** and observe the information displayed in Spark UI.
...
7. Return to the Synapse Analytics Monitoring page for your Spark application.

8. To look closer at any individual stage, you can use the **Job IDs** drop-down to select the stage number. In this case, select the drop-down and choose **5** from the list.
...
9. This view isolates the specific stage within the graphical view.

    ![Stage 5 is displayed.](media/ex02-monitor-activities-spark-applications-stage-5.png "Synapse Analytics Monitor")

10. Return the view to all stages by selecting **All job IDs** in the job ID drop-down list.
...
11. Within the graph section, you also have the ability to **Playback** the Spark application.
...
12. Running a Playback allows you to observe the time required to complete each stage, as well as review the rows read or written as the job progresses.
...
13. You can also preform a playback on an individual stage. Returning to a view of only Stage 5, the **Playback** button shows the rows written at this stage, along with the progress of reads and writes.
...
14. You can also change the view to see which stages were involved in read and write activities. In the **Display** drop-down, select **Read**.
...
15. In the read graph, stages that involved read tasks are highlighted in orange.
...
16. Next, select **Written** from the Display drop-down list, and observe the stages where data was written.
